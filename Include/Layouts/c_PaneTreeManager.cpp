...
namespace AppCore {
    ...
    PaneTreeManager::~PaneTreeManager() {
        for ( int i = 0; i < SUIObjects.size(); ++i ) {
            SUIObjects[i]->~SUI_PipelineBase();
        }
    ...
    SUI_PipelineBase* PaneTreeManager::GetSUI( string inName ) {
        auto iter = SUIMap.find( inName );
        if ( iter == SUIMap.end() ) {
            std::cout << "SUI Object not found: " << inName << std::endl;
            assert( iter != SUIMap.end() );
        }
        return SUIObjects[iter->second];
    }
    ...
    void PaneTreeManager::InitPipelines( size_t resource_count ) {
        for ( int i = 0; i < SUIObjects.size(); ++i ) {
            SUIObjects[i]->Init( resource_count );
        }
    ...
    vector<SUI_PipelineBase*> PaneTreeManager::GetSUIVector( PaneTree *inPT ) {
        vector<SUI_PipelineBase*> output;
        for( auto & node : inPT->Nodes ) {
            string node_name = node.Name;
            if ( GetPaneType( node_name ) == 1 ) {
                output.push_back( GetSUI( node_name ) );
            }
        }
        return output;
    }
    ...
    void PaneTreeManager::UpdateModes( PaneTree *inPT ) {
         for( auto & node : inPT->Nodes ) {
            ...
            } else if ( GetPaneType( node_name ) == 1 ) {
                GetSUI( node_name )->UpdateMode( node.Mode );
            }
        }
    }
    void PaneTreeManager::UpdateDimensions( PaneTree *inPT ) {
        for( auto & node : inPT->Nodes ) {
            ...
            if ( GetPaneType( node_name ) == 0 ) {
                ...
                GetSUI( node_name )->UpdateDimensions( (float) node.X, (float) node.Y, (float) node.W, (float) node.H );
            }
        }
    }
